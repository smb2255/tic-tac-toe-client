...
var resetGameBoard = function resetGameBoard() {
  for (var i = 0; i < 9; i++) {
    $('#' + i).html('');
    $('#turn-message').text(' ');
    $('#space-taken').hide();
...
var resetGame = function resetGame(event) {
  createGame();
  resetGameBoard();
};

var player = function player(boxNum) {
...
var onCreateUser = function onCreateUser(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  // console.log(data)
  // console.log(config.apiOrigin)
  api.create(data).then(ui.createUserSuccess).catch(ui.createUserFailure);
};
...
var onChangePass = function onChangePass(event) {
  ...
  // console.log(config.apiOrigin)
  api.changePass(data).then(ui.changePassSuccess).catch(ui.changePassFailure);
};

var startGame = function startGame(event) {
  api.startGame().then(ui.startGameSuccess).catch(ui.startGameFailure);
};
...
var addHandlers = function addHandlers() {
  var boxClick = function boxClick(event) {
    event.preventDefault();
    // console.log(this.id)
...
var boardFull = function boardFull(gameArray) {
...
var onGetStats = function onGetStats(event) {
  event.preventDefault();
  api.getStats().then(ui.getStatsSuccess).catch(ui.getStatsFailure);
};

var onUpdateGame = function onUpdateGame(event) {
  if (checkWinner() === true || boardFull(gameArray) === true) {}
  api.updateGame().then(ui.updateGameSuccess).catch(ui.updateGameFailure);
};
...
var getTotalGames = function getTotalGames() {
  return $.ajax({
    url: config.apiOrigin + '/games',
    method: 'GET',
    headers: {
...
var createUserSuccess = function createUserSuccess() {
  $('#sign-up-msg').html('<p> Sign up success! </p>');
  $('#credentials_SU').hide();
};

var createUserFailure = function createUserFailure() {
...
var getStatsSuccess = function getStatsSuccess(data) {
  store.stats = data;
  $('#stats-button').text('You have played' + ' ' + store.stats.game.length + ' ' + 'games');
};

var getStatsFailure = function getStatsFailure(error) {
  console.error(error);
};
